...
struct SupportedLocalesTests {
    ...
    /// Phase 6 (`en-US` baseline) + Phase 7 (Major Western European) + Phase 8 (Dutch, Nordics & Greek).
    @Test func containsExactlyTheShippedTiers() {
        #expect(Set(SupportedLocales.all.map(\.id)) == ["en-US", "en-GB", "es", "fr", "de", "it", "pt-BR", "nl", "da", "el", "sv", "fi", "nb"])
    }
    ...
    @Test func matchingDeviceFallsBackToTheFirstEntryWhenTheDeviceLanguageIsntShipped() {
        // Polish isn't shipped yet (Phase 10) — G4b: fall back to the first entry, never nil.
        #expect(SupportedLocales.matchingDevice(languageCode: "pl").id == "en-US")
    }
