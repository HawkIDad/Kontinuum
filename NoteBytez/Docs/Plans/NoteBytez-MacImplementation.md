...
- **In scope:** one native macOS destination, built from the same source tree and file list as iOS/iPadOS, sharing the same CloudKit container (`iCloud.com.kwicksync.NoteBytez`) so a library syncs across all three platforms with no extra plumbing.
...
- [x] Created [NoteBytez-macOS.entitlements](../../NoteBytez-macOS.entitlements): same `iCloud.com.kwicksync.NoteBytez` container + `CloudKit` service as [NoteBytez.entitlements](../../NoteBytez.entitlements), **without** `com.apple.developer.default-data-protection`. Wired in via `"CODE_SIGN_ENTITLEMENTS[sdk=macosx*]" = NoteBytez/NoteBytez-macOS.entitlements;` on the app target (base `CODE_SIGN_ENTITLEMENTS` still points at the iOS file) — confirmed via a real build log: the packaged entitlements for the macOS build contain the iCloud/CloudKit keys and *not* the data-protection key that broke the prior Catalyst attempt
...
- [ ] Cross-platform CloudKit sync smoke test: a note created on iPhone/iPad appears on Mac through the shared `iCloud.com.kwicksync.NoteBytez` private container (needs a live iCloud account — same caveat already logged against MVP Plan Phases 12–14)
