<!-- NoteBytez20260906v1-NameChange.md -->
<!--
    Product rename to NoteBytez (surface / user- and plugin-visible pass).
...
**NoteBytez**. Every place a user (or a plugin author) can *see* the old name "NoteBytez" must
...
identifier, CloudKit container, and the per-file header comments — keeps the name "NoteBytez".
...
1. **No user-visible "NoteBytez" anywhere in the running app.** Every on-screen string
...
2. **The app's on-device name is "NoteBytez".** `CFBundleDisplayName` is set to `NoteBytez` for
   ...
   `NoteBytez`; only the display string changes).
3. **All Markdown documentation reads "NoteBytez".** Every `.md` file under `NoteBytez/` that is
   not `z_`-prefixed (excluded per repo `CLAUDE.md`) has its body text updated, and every
   `NoteBytez*.md` file is renamed to `NoteBytez*.md` with the original datestamp/version suffix
...
4. **The plugin SDK is renamed.** The JavaScript global `noteBytez.*` becomes `noteBytez.*`
...
5. **Name-bearing asset files are renamed.** `images/logos/NoteBytez-V1.jpg` →
...
6. **Build and tests stay green.** `xcodebuild build` succeeds for iOS and macOS; the full
   `NoteBytezTests` suite passes (with `PluginBridgeTests` updated to `noteBytez.*`); no
   regression in `NoteBytezUITests`.
7. **Deliberate retentions are documented, not silently skipped.** A "Legacy Identifiers"
   ...
   `PRODUCT_MODULE_NAME = NoteBytez`, since `PRODUCT_NAME` is now `NoteBytez`), targets,
   scheme blueprint, `.xcodeproj`, source directory, `NoteBytezApp.swift`, `*.entitlements`
   filenames, bundle identifier `com.g9Consulting.NoteBytez`, CloudKit container
   `iCloud.com.g9Consulting.Kontinuum`, the `notebytez-private-changes` /
   `notebytez-shared-changes` subscription IDs, `Notification.Name` raw values, `DispatchQueue`
   labels, the `Logger` subsystem fallback string, and the `// NoteBytez` file-header line all
...
(e.g. the `noteBytez` JS global uses lowerCamelCase to match the old `noteBytez` global's
casing).
...
| # | Question | Resolution |
|---|---|---|
| **R1** | How deep does the rename go beyond on-screen text? | **Visual + docs only.** Leave all code identifiers, the Swift module/target/scheme/`.xcodeproj`/source-dir names, `Notification.Name` raw values, `DispatchQueue`/log labels, and the `// NoteBytez` file-header comments untouched. The plugin SDK API is the one carve-out (R4) because plugin authors are users of it. |
| **R2** | CloudKit container `iCloud.com.g9Consulting.Kontinuum` and bundle ID `com.g9Consulting.NoteBytez` — containers can't be renamed. | **Keep both as-is.** Zero data migration, no App Store Connect / provisioning churn. A code comment at each definition site notes the legacy name. |
...
| **R4** | Plugin `noteBytez.*` JS global + `NoteBytez*.md` doc filenames. | **Hard rename both.** `noteBytez.*` → `noteBytez.*` with no deprecation alias; all `NoteBytez*.md` files renamed to `NoteBytez*.md` and cross-links fixed. Justified by pre-launch status — no shipped plugin, no external doc links to break. |
...
| **DR2** | Historical dated plan docs (`NoteBytez20260627v2-Security.md`, etc.) | Rename file + replace the name in body text; do **not** rewrite the decisions/history they record. | SF#3 + R4 say rename; the documents' value as a decision record is preserved by leaving their substance intact. |
| **DR3** | `NoteBytez-V1.jpg` logo asset | Rename to `NoteBytez-V1.jpg` (placeholder — final logo art is still in design, OQ2). Not referenced from code, so the rename is safe and lossless. | Housekeeping. |
| **DR4** | Repo-root `NoteBytez/CLAUDE.md` | In scope for the prose sweep; infra-identifier lines (`iCloud.com.g9Consulting.Kontinuum`) left verbatim. Workspace-level `../CLAUDE.md` is shared across projects and out of scope except any line naming this app. | `CLAUDE.md` is a Markdown document (SF#3) but is also build/infra instruction — treat identifiers as identifiers. |
| **DR5** | `z_`-prefixed Markdown | Excluded from the sweep. | Repo `CLAUDE.md`: "Ignore markdown files beginning with 'z_'". |
| **DR6** | Swift local var `let noteBytez` in `PluginBridge.swift` | Rename to `noteBytez` for readability alongside the JS-facing rename; it is not an API surface. | Cosmetic; keeps the file self-consistent after R4. |
...
| **OQ2** | Does the AppIcon / logo art contain the "NoteBytez" wordmark? | Originally deferred (art undefined). **Resolved 2026-09-07:** the product owner supplied NoteBytez artwork; the app icon and iOS launch screen were built from it in Phase 4. No "NoteBytez" anywhere in the art. |
...
| **OQ4** | Any pilot plugin written against `noteBytez.*` we own? | **No plugins created.** Phase 2 touches only `PluginBridge.swift`, `PluginBridgeTests`, and `PluginSDK.md`. |
| **OQ5** | External references to `NoteBytez*.md` doc paths? | **None.** Phase 3 link-fixing is entirely repo-internal. |
...
> **Executed 2026-09-06.** All phases complete. Full `NoteBytezTests` suite 727/727 green on the
...
- [x] `CFBundleDisplayName = NoteBytez` added to `NoteBytez/Info.plist` **and**
  `NoteBytez/Info-macOS.plist` directly (not via `INFOPLIST_KEY_*` — Xcode's generated-plist
...
- [x] `PRODUCT_NAME = NoteBytez` on the app target (both configs) — so `CFBundleName`,
  ...
  **macOS app menu** now shows "About NoteBytez" / "Quit NoteBytez". Paired with
  `PRODUCT_MODULE_NAME = NoteBytez` (explicit) so the Swift module name is **not** dragged along
  — `@testable import NoteBytez` in ~80 test files still resolves. `NoteBytezTests` `TEST_HOST`
...
- [x] → verify: `Scripts/verify-rename.sh` passes; simulator screenshots confirm "NoteBytez" on
  ...
  `CFBundleIdentifier` = `com.g9Consulting.NoteBytez` (retained), `PRODUCT_MODULE_NAME` =
  `NoteBytez` (retained).

### Phase 2 — Plugin SDK hard rename (breaking)
- [x] `PluginBridgeTests.swift` — all JS `entryScript` snippets `noteBytez.*` → `noteBytez.*`
  (ran red first). Also updated `PluginViewModelTests.swift` (executes a script — would have
  failed), and the `noteBytez.addCommand('x')` fixture strings in `PluginDALTests.swift`,
  `BackupDALTests.swift`, `SyncMappingTests.swift` for consistency.
- [x] `PluginBridge.swift` — JS global string `"noteBytez"` → `"noteBytez"`; local
  `let noteBytez` → `noteBytez` (**DR6**); doc comments updated. `PluginViewModel.swift:58`
  comment updated.
- [x] `Docs/PluginSDK.md` — retitled "NoteBytez Plugin SDK (Preview)", all 19 `noteBytez` →
  `noteBytez`, prose updated, top-of-file **Breaking change** note added.
- [x] → verify: full suite green; no `noteBytez` (lowercase) remains in code except the retained
  subscription IDs and the `.noteBytez*` `Notification.Name` comment.

### Phase 3 — Documentation sweep
- [x] Renamed all 15 `NoteBytez*.md` plan docs → `NoteBytez*.md` (datestamp/version preserved;
  the `- HOLD -` marker on the MultiLanguage doc kept).
- [x] Guarded body-text sweep across 25 in-scope `.md` files (`NoteBytez/**` non-`z_` +
  repo-root `CLAUDE.md`; `NoteBytez.md` itself excluded — it authors both names): `NoteBytez`
  → `NoteBytez` **except** `com.g9Consulting.NoteBytez`, `iCloud.com.g9Consulting.Kontinuum`,
  `noteBytez-*-changes`, `import NoteBytez`, `-scheme/-project/-target NoteBytez`,
  `NoteBytez{Tests,UITests,App,CIGate}`, `NoteBytez.{xcodeproj,xcscheme,app,entitlements}`,
  `NoteBytez-macOS.entitlements`, `NoteBytezSecurity.xctestplan`, and the `NoteBytez/` source
  path prefix (retained per R1). Link text `](NoteBytez*.md)` tracked the file renames in the
...
- [x] Fixed Swift **doc-comment** references to the renamed plan docs (50 `.swift` files, e.g.
  `/// Per NoteBytez20260824v1-Templates.md` → `NoteBytez…`) — pointer fixes the rename
  created, not an identifier change. Headers (`//  NoteBytez`), module, and `NoteBytezApp`
  untouched.
- [x] Repo-root `CLAUDE.md`: its only "NoteBytez" is the retained container ID — no change (DR4
  satisfied trivially).
- [x] → verify: `Scripts/verify-rename.sh` (b) passes; the only `NoteBytez` left in docs is the
...
### Phase 4 — Assets
- [x] `NoteBytez/images/logos/NoteBytez-V1.jpg` → `NoteBytez-V1.jpg` (placeholder, unreferenced
  in code).
...
- [x] → verify: clean iOS + macOS builds green; `NoteBytezTests` 727/727; icon renders on the
...
- [x] `xcodebuild test -only-testing:NoteBytezTests` — **727/727 pass** on iPhone 17 Pro
  (iOS 26 sim).
...
- [x] `ARCHITECTURE.md` → new **"Legacy Identifiers"** section enumerating every retained
  `NoteBytez` identifier and why; points at `Scripts/verify-rename.sh` as the enforcement.
...
- **`PRODUCT_NAME = NoteBytez` (added on product-owner instruction, after the first pass).**
  ...
  crossing R1's line, `PRODUCT_MODULE_NAME = NoteBytez` was pinned explicitly: without it,
  `PRODUCT_NAME` also becomes the Swift module name and every `@testable import NoteBytez`
  breaks (seen and fixed during this change). `NoteBytezTests` `TEST_HOST` was repointed to
  `NoteBytez.app`. Net: the *product/bundle* is NoteBytez; the *module, target, scheme
  blueprint, project, bundle ID, container* stay NoteBytez. `NoteBytez.xcscheme`'s
  `BuildableName` auto-updated to `NoteBytez.app` (Xcode owns that field; `BlueprintName`
  stays `NoteBytez`). One historical build-log quote in `NoteBytez-MacImplementation.md`
  still says "validated as `NoteBytez.app`" — left as a dated record (`CLAUDE.md` §3).
- **Scope extension (kept minimal):** ~50 `.swift` files had doc-comment references to the
  renamed plan docs (`/// Per NoteBytez20260…md`). These were updated — leaving them would have
...
green; `NoteBytezTests` 727/727; screenshot pass done; `ARCHITECTURE.md` "Legacy Identifiers"
section added. `PRODUCT_NAME = NoteBytez` applied (macOS app menu + built `NoteBytez.app`),
with `PRODUCT_MODULE_NAME` pinned to `NoteBytez` so the module name is retained. No open
