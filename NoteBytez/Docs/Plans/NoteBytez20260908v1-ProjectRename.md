...
<!--
    Rename the Xcode project and every remaining internal reference NoteBytez -> NoteBytez.
    Supersedes the deliberate "keep internal identity as NoteBytez" decision in
...
documentation renamed from **NoteBytez** to **NoteBytez**, **so that** the codebase carries one
...
users and plugin authors could see and explicitly retained the internal `NoteBytez` identity
...
to a new one — and it becomes the only `NoteBytez` string allowed to remain anywhere in the
repository.
...
3. **Bundle identifiers change.** `com.g9Consulting.NoteBytez` → `com.g9Consulting.NoteBytez`
...
6. The **GitHub repo rename** (`HawkIDad/NoteBytez` → `HawkIDad/NoteBytez`) is an account/settings
...
`NoteBytez`. **Check:** `xcodebuild -list -project NoteBytez.xcodeproj` + `git ls-files | grep -i noteBytez`.
...
### SF3 — Zero `NoteBytez` outside the container
No case-insensitive `noteBytez` in any git-tracked file **except** the literal
`iCloud.com.g9Consulting.Kontinuum`. **Check:** the inverted `Scripts/verify-rename.sh` exits 0;
`git grep -i noteBytez` returns only container-string lines.
...
| **R2** | Depth undefined — does "all references" include the Swift module, runtime strings, and 346 file headers? | **Everything.** Module name (`import NoteBytez`), `NoteBytezApp` → `NoteBytezApp`, `NoteBytezUITestCase` → `NoteBytezUITestCase`, all `Notification.Name` raw values, `DispatchQueue` labels, `Logger` subsystem + fallback, and the `//  NoteBytez` header line in every `.swift`. Only the CloudKit container survives. |
...
| **R4** | Other embedded identifier strings — StoreKit product IDs, `.storekit` filename, CloudKit zone subscription IDs. | **Rename all to NoteBytez.** Product IDs → `com.g9Consulting.NoteBytez.sub.*` / `.entitlement`; `NoteBytez.storekit` → `NoteBytez.storekit`; subscription IDs → `notebytez-*-changes`. Safe pre-launch — nothing server-side is authoritative and data is disposable. |
...
| **R6** | Git repo + GitHub — in scope, or project-internal only? | **In scope.** Rename the local working-copy folder `NoteBytez/` → `NoteBytez/` and the GitHub repo `HawkIDad/NoteBytez` → `NoteBytez` (GitHub keeps redirects), then `git remote set-url origin …/NoteBytez.git`. |
| **R7** | Historical documentation — rewrite the dated decision records, or preserve them? | **Full sweep, all docs.** Every non-`z_` `.md` including the dated historical plans; `Docs/Plans/Projects/NoteBytez.md` → `…/NoteBytez.md`; `ARCHITECTURE.md` "Legacy Identifiers" replaced with a short "Retained name" note. Historical plans get a one-line "superseded by NoteBytez20260908v1-ProjectRename.md" banner but their decisions are not rewritten. |
| **R8** | No verification method / success gate. | **Inverted `verify-rename.sh` + build/test gates:** `xcodebuild` iOS + macOS green; full `NoteBytezTests` at baseline; `NoteBytezUITests` per the CI Gate plan; Simulator smoke + CloudKit connect; `verify-rename.sh` exit 0; `git grep -i noteBytez` shows only the container. Encoded as SF1–SF7 and Phase 8. |
| **R9** | `verify-rename.sh` currently enforces the *opposite* (allow-lists all retentions). | **Invert it**, keeping the filename. New rule: exit 1 on any case-insensitive `noteBytez` in `git ls-files` output except the container string (and the script's own allow-list line). Header comment updated. |
...
| **DD2** | `Notification.Name` raw values | `noteBytez*` → `noteBytez*` (lowerCamelCase). | `CLAUDE.md` §5 naming; matches the existing casing of these constants. |
| **DD3** | CloudKit zone subscription IDs | `notebytez-private-changes` / `notebytez-shared-changes` → `notebytez-private-changes` / `notebytez-shared-changes` (kebab-case). | Preserves the existing token shape; only the name changes. |
| **DD4** | Verify script | Keep the filename `Scripts/verify-rename.sh`; invert the logic; allow-list = exactly `iCloud\.com\.g9Consulting\.NoteBytez`. | Interview chose "invert", not "new name". One allow-list entry keeps the gate honest. |
| **DD5** | Dated historical plans | Swept in place (body text) with a "superseded" banner; **not** renamed again — they already carry `NoteBytez…` date-prefixed filenames from the 2026‑09‑06 rename. Only `Docs/Plans/Projects/NoteBytez.md` is `git mv`d. | Their filenames are already correct; only `Projects/NoteBytez.md` is stale. |
...
| **DD7** | Pre-existing drift | **Noted, not fixed** (`CLAUDE.md` §3): `ARCHITECTURE.md` "Legacy Identifiers" cites `NoteBytezSecurity.xctestplan` but the file is `NoteBytezEntitlement.xctestplan`; app `MARKETING_VERSION` `0.1.0` vs test targets `1.0`. The "Legacy Identifiers" rewrite fixes the first as a side effect. |
...
asserts on a literal `NoteBytez` string is updated in the same phase that changes the string
...
- Capture baseline: `xcodebuild build` (iOS Sim + macOS) and `xcodebuild test
  -only-testing:NoteBytezTests` — record pass/fail and the test count.
- Commit the current full `noteBytez` inventory (`git grep -in noteBytez`) as
...
- `git mv`: `NoteBytez` → `NoteBytez`, `NoteBytezTests` → `NoteBytezTests`, `NoteBytezUITests`
  → `NoteBytezUITests`, `NoteBytez.xcodeproj` → `NoteBytez.xcodeproj`,
  `NoteBytez{,-macOS}.entitlements` → `NoteBytez{,-macOS}.entitlements`,
  `NoteBytezCIGate.xctestplan` → `NoteBytezCIGate.xctestplan`,
  `NoteBytezEntitlement.xctestplan` → `NoteBytezEntitlement.xctestplan`,
  `NoteBytez/StoreKit/NoteBytez.storekit` → `NoteBytez/StoreKit/NoteBytez.storekit`,
  `xcshareddata/xcschemes/NoteBytez.xcscheme` → `NoteBytez.xcscheme`.
- Edit `project.pbxproj`: target `name` / `productName` (×3), `remoteInfo` (×2),
  `.xctest` product refs + comments, `path = NoteBytez*` on the 3 synchronized groups,
...
- Identify and handle the 1 `.plist` with a `noteBytez` match (`git grep -il noteBytez
...
- `NoteBytezApp.swift` → `NoteBytezApp.swift`; `struct NoteBytezApp` → `NoteBytezApp` (+ the
  `@main` attribute site and any `NoteBytezApp.init` / preview refs).
- `NoteBytezTests/NoteBytezTests.swift`, `NoteBytezUITests/NoteBytezUITests.swift`,
  `NoteBytezUITestsLaunchTests.swift`, `Support/NoteBytezUITestCase.swift` → `NoteBytez…`;
  `class NoteBytezUITestCase` → `NoteBytezUITestCase` (+ every subclass reference).
- `@testable import NoteBytez` → `@testable import NoteBytez` across all ~80 test files;
  any plain `import NoteBytez`.
...
- `Notification.Name` raw values `noteBytez*` → `noteBytez*` (definitions in the extension +
  every `.noteBytez…` call site) (DD2).
...
- StoreKit product-ID constants → `com.g9Consulting.NoteBytez.sub.monthly` / `.sub.annual` /
  `.entitlement`; mirror inside `NoteBytez.storekit` (product IDs + any `NoteBytez` name field)
  (DD9).
- CloudKit zone subscription IDs → `notebytez-private-changes` / `notebytez-shared-changes` (DD3).
- Remaining strays: `notebytez2` and any helper `noteBytez*` func/var names → investigate,
  rename.
- → **verify:** suite green; `git grep -i noteBytez -- '*.swift'` returns **only** a container
  constant line if one exists (else nothing).

### Phase 4 — File headers (SF3)
- Scripted replace of the header line `//  NoteBytez` → `//  NoteBytez` across
  `git ls-files '*.swift'` (~346 files). Leave the
  `// © Copyright, 2026 David L. Collison, All Rights Reserved.` line untouched.
- → **verify:** build green iOS + macOS; `git grep -n '^//  NoteBytez'` empty.

### Phase 5 — Documentation full sweep (SF7)
- Every non-`z_` `.md` under the repo (incl. `NoteBytez/CLAUDE.md`, `ARCHITECTURE.md`, all
  `Docs/Plans/**`, `Wiki/**`): `NoteBytez` → `NoteBytez` **except**
  `iCloud.com.g9Consulting.Kontinuum`.
- `git mv Docs/Plans/Projects/NoteBytez.md Docs/Plans/Projects/NoteBytez.md`; fix inbound links.
- `ARCHITECTURE.md`: replace the "## Legacy Identifiers" section with a short **"Retained
  name"** paragraph — the CloudKit container is the only `NoteBytez` string left, and
...
- → **verify:** markdown link-check clean; `git grep -i noteBytez -- '*.md'` returns only
  container lines.

### Phase 6 — Repository (SF6)
- **Maintainer, in GitHub settings:** rename `HawkIDad/NoteBytez` → `HawkIDad/NoteBytez` (DD6).
- `git remote set-url origin https://github.com/HawkIDad/NoteBytez.git`; `git fetch` to confirm.
- Quit Xcode; `mv ~/DevProjects/xCodeWorkspace/NoteBytez ~/DevProjects/xCodeWorkspace/NoteBytez`;
...
- Rewrite `Scripts/verify-rename.sh`: `set -euo pipefail`; iterate `git ls-files` (skip
  ...
  any case-insensitive `noteBytez` match except lines matching
  `iCloud\.com\.g9Consulting\.NoteBytez` or the script's own `ALLOW=` definition (DD4). Update
  the header comment to describe the inverted intent.
- Add a canary (`// noteBytez` in a scratch file), confirm the script exits 1, remove it.
